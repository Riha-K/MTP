...
2. Extract:
   ```powershell
   cd e:\MTP\earth2\EarthDial-main\data\baresoil_s1\ai4lcc\multisenge
   tar -xzf s1.tgz
   ```
...
|--------|------|----------|
| Train shard | `data/baresoil_s1/shards/ai4lcc_ge_train_train/` | ~22k QA (8157 patches × 3 templates) |
| Val shard | `data/baresoil_s1/shards/ai4lcc_ge_train_val/` | ~10% held-out |
| Bench JSONL | `data/baresoil_s1/bench/v0.1/ai4lcc_val.jsonl` | Classification + binary bare QA |
...
```powershell
python -c "from baresoil.ai4lcc import iter_patches; p=iter_patches('data/baresoil_s1/ai4lcc/multisenge/labels'); print(len(p), p[0])"
```

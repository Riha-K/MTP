...
## 3. Papers to read — order + objective
...
| Done | Detail | Proof |
|------|--------|-------|
| S1 VH + OCSGE 14-class templates | classify + 2-turn dialogue | `baresoil/` |
...
## 6. Phase 1 — Write-up (next)

| Step | Action |
|------|--------|
| **1.1** | Master results table (§8) |
| **1.2** | Bench spec (tasks, 14 classes, 70/30, metrics) |
| **1.3** | 10 figure patches |
| **1.4** | 3–5 failure cases |
| **1.5** | Draft Intro + Method + Results |
| **1.6** | **MultiSenNA** predict + eval with `LULCDial_S1_v0.1` |

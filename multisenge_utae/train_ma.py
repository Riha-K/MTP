"""Train MA-UTAE (Task M) with optional Task H agents (A1/A2).

Phases (same names as stock U-TAE):
  --mode head  = P4 (freeze dual encoders + L-TAEs; train fusion/decoder/heads)
...
Example smoke:
  python -m multisenge_utae.train_ma \\
    ...
    --fusion gated --use-a1 --use-a2 \\
    --max-train 4 --max-val 2 \\
    --out-dir multisenge_utae/checkpoints/ma_c6_smoke
"""
...
from multisenge_utae.heads import hierarchical_loss
from multisenge_utae.models.ma_utae import MAUTAE


def _forward_logits(model: MAUTAE, x: torch.Tensor, bp: torch.Tensor):
  out = model(x, batch_positions=bp)
  if isinstance(out, tuple):
    return out[0], out[1] if len(out) > 1 and isinstance(out[1], dict) else {}
  return out, {}


@torch.no_grad()
def evaluate(model: MAUTAE, loader: DataLoader, device: torch.device, num_classes: int) -> dict:
  model.eval()
  ...
  for batch in loader:
    ...
    logits, _ = _forward_logits(model, x, bp)
...
def train_one_epoch(
  ...
  for i, batch in enumerate(loader):
    ...
    bp = batch_positions(x.shape[0], device)
    logits, aux = _forward_logits(model, x, bp)
    if aux:
      loss, _parts = hierarchical_loss(
          logits, mask, aux, criterion, lambda_a1=lambda_a1, lambda_a2=lambda_a2
      )
    else:
...
def build_model(args, n_cls: int, device: torch.device) -> MAUTAE:
  model = MAUTAE(
      num_classes=n_cls,
      fusion=args.fusion,
      fuse_skips=not args.no_fuse_skips,
      use_a1=args.use_a1,
      use_a2=args.use_a2,
  ).to(device)
  ...
  print(
      f"MA-UTAE fusion={args.fusion} a1={args.use_a1} a2={args.use_a2} "
      f"mode={args.mode} trainable={trainable:,} / {total:,}"
  )
  return model
...
def _run_eval(args, records: list[PatchRecord], n_cls: int) -> int:
  ...
  use_a1 = bool(meta.get("use_a1", args.use_a1))
  use_a2 = bool(meta.get("use_a2", args.use_a2))
  fuse_skips = not bool(meta.get("no_fuse_skips", args.no_fuse_skips))
  ...
  model = MAUTAE(
      num_classes=n_cls,
      fusion=fusion,
      fuse_skips=fuse_skips,
      use_a1=use_a1,
      use_a2=use_a2,
  ).to(device)
  model.load_state_dict(ckpt["model"])
  print(f"eval ckpt={ckpt_path} split={args.eval_split} n={len(ds)} classes={n_cls} fusion={fusion}")
...
def main() -> int:
  p = argparse.ArgumentParser(description="Train MA-UTAE (Task M) ± hierarchical A1/A2 (Task H)")
  ...
  p.add_argument("--fusion", type=str, default="gated", choices=["gated", "concat"])
  p.add_argument("--no-fuse-skips", action="store_true", help="Fuse bottleneck only (S2 skips)")
  p.add_argument("--use-a1", action="store_true", help="P2 agent: UF vs rest coarse head")
  p.add_argument("--use-a2", action="store_true", help="P2 agent: Dense vs Sparse head")
  p.add_argument("--lambda-a1", type=float, default=0.3)
  p.add_argument("--lambda-a2", type=float, default=0.2)
  ...
  p.add_argument("--out-dir", type=Path, default=Path("multisenge_utae/checkpoints/ma_c6_gated_head_v0"))
  ...
  for epoch in range(1, args.epochs + 1):
    ...
    loss = train_one_epoch(
        ...
        lambda_a1=args.lambda_a1,
        lambda_a2=args.lambda_a2,
    )
